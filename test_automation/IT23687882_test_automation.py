...
DEFAULT_EXCEL_CANDIDATES = [
    ...
    str(SCRIPT_DIR / "IT23687882_Assignment 1 - Test cases.xlsx"),
]
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    def match(name: str) -> int | None:
        ...
        name_tokens = set(re.findall(r"[a-z0-9]+", str(name).lower()))
        if not name_tokens:
            return None
        ...
        best_score = None
        for i, v in indexed:
            header_tokens = set(re.findall(r"[a-z0-9]+", v.lower()))
            if not header_tokens:
                continue
            if name_tokens == header_tokens:
                return i
            if name_tokens.issubset(header_tokens):
                # Prefer the closest header label (fewest extra tokens, then shortest normalized form).
                score = (len(header_tokens) - len(name_tokens), len(_normalize_header(v)))
...
def _click_transliterate(page, action_locator=None) -> bool:
    candidates = []
    if action_locator is not None:
        candidates.append(action_locator)
    candidates.append(page.get_by_role("button", name=re.compile(r"^Transliterate$", re.IGNORECASE)).first)
    candidates.append(page.locator("button:has-text('Transliterate')").first)
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                clicked = _click_transliterate(page, action_locator)
                if not clicked:
                    try:
                        # Some UI versions can trigger transliteration on Enter.
                ...
                for i in range(tries):
                    ...
                    if not prev_output or current != prev_output or i == tries - 1:
                        break
                    page.wait_for_timeout(max(0, int(args.retry_wait_ms)))
                
                if not actual_output:
                    if _click_transliterate(page, action_locator):
                        page.wait_for_timeout(max(0, int(args.retry_wait_ms)))
                        actual_output = _read_output(is_chat, output_locator)

                if not actual_output:
                    raise RuntimeError("Output is empty after retries.")
